...
def check_angle(angle: Any, lower_bound: float = -inf, upper_bound: float = inf):
    """Raise an exception when the provided value is not valid for a
...
def check_type(
    ...
    """Raise an exception when the provided value is not valid for a given type.
...
def check_range(value: Any, lower_bound: float, upper_bound: float, parameter_name: str):
    """Raise an exception when the provided value is not valid for a range, being
...
def area_message(error_message_key: str, width: float, height: float) -> str:
    """Emit a warning indicating that the graphic cannot be shown or saved
...
def check_graphic_size(rounded_size: ISize):
    """Raise an exception when the provided size is not valid for a
